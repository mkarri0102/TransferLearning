...
def test_svm(clf, features, labels):
...
def main(args, result_dict):
    ...
    for i in tqdm(range(n_problems)):
        ...
        if args.classifier_type in ['linear', 'rbf', 'poly', 'sigmoid']:
            labels_query = np.array(labels_query)
            clf = svm.SVC(kernel=args.classifier_type)
            clf.fit(features_query, labels_query)
            accuracy_test = test_svm(clf, features_query, labels_query)
